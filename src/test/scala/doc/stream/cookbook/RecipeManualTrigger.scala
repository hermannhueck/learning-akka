...
class RecipeManualTrigger extends RecipeSpec {
  ...
  "Recipe for triggering a stream manually" must {
    ...
    "work" in {
      ...
      val pub: TestPublisher.Probe[Trigger] = TestPublisher.probe[Trigger]()
      val sub = TestSubscriber.manualProbe[Message]()
      val triggerSource: Source[Trigger, NotUsed] = Source.fromPublisher(pub)
      val sink: Sink[Message, NotUsed] = Sink.fromSubscriber(sub)
